 ...
 * ninguno de esos referencia ninguna dinámica en particular) y sobreescribe
 * únicamente los métodos que dependen de codigo_azul_momentos()/
 * CodigoAzulDimensiones. Mismo patrón que VolverACasaAnalisisModel.
 */
class CodigoAzulAnalisisModel extends RespuestaMomentoModel
{
    /**
     * Qué opciones del Momento 5 reflejan que la persona sintió que su
     * opinión llegó con claridad a la decisión final. Los 5 roles de
     * "Código Azul" comparten el mismo vocabulario de cierre.
     */
    ...
    private function calcularDimensiones(array $integrantes): array
    {
        ...
        foreach ($integrantes as $integrante) {
            foreach ($integrante['valores'] as $momento => $valor) {
                ...
                $puntaje = CodigoAzulDimensiones::puntaje($momento, $integrante['role'], $valor);
                if ($puntaje !== null) {
        ...
        $dimensiones = [
            CodigoAzulDimensiones::NOMBRES_DIMENSION[1] => $p1 === null ? '—' : CodigoAzulDimensiones::nivel($p1),
            CodigoAzulDimensiones::NOMBRES_DIMENSION[2] => $p2 === null ? '—' : CodigoAzulDimensiones::nivel($p2),
            CodigoAzulDimensiones::NOMBRES_DIMENSION[3] => $p3 === null ? '—' : CodigoAzulDimensiones::nivel($p3),
            CodigoAzulDimensiones::NOMBRES_DIMENSION[4] => $p4 === null ? '—' : CodigoAzulDimensiones::nivel($p4),
            'Persistencia ante presión' => ($p3 === null || $p4 === null)
                ? '—'
                : CodigoAzulDimensiones::nivel($p4 - $p3 + 1),
        ];
