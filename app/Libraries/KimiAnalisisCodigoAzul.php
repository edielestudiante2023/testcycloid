...
/**
 * Analisis de patrones de comunicacion para el cierre reflexivo de "Codigo
 * Azul", usando Kimi (Moonshot AI). Mismo patron que KimiAnalisis.php (El
 * Meridian) y KimiAnalisisVolverACasa.php — es un texto de apoyo para el
 * facilitador, NO un diagnostico psicologico clinico de nadie.
 *
 * La "radiografia" (dimensiones) se calcula matematicamente en
 * CodigoAzulAnalisisModel::radiografiaPorEquipo() y se le pasa a Kimi como
 ...
 */
class KimiAnalisisCodigoAzul
{
    private const SYSTEM_PROMPT = <<<'PROMPT'
        Eres un facilitador experto en dinamicas de liderazgo y comunicacion organizacional. Analizas los
        datos de un ejercicio de simulacion (un equipo de emergencias de un hospital ficticio que debe
        integrar señales distintas antes de que la presion del tiempo cierre una decision clinica sin ellas)
        para preparar al facilitador humano antes de su conversacion de cierre real con el equipo.
        ...
        3. Sostener o ceder ante presion: alguien que sostuvo su lectura cuando se le presiono para
           resolver rapido, o que cedio dando la version mas comoda o mas rapida.
        ...
        PUNTO DE QUIEBRE: la etapa de mayor presion NO introduce ningun dato clinico nuevo — el medico
        tratante ya habia inclinado su decision antes de preguntar, y solo pide "¿tienes algo que cambie
        esto?". Por eso, cualquier cambio de posicion en esa etapa es atribuible limpiamente a la presion de
        la autoridad, no a informacion nueva. La radiografia te indica si la capacidad del equipo de
        sostener su lectura bajo esa presion fue alta, media o baja, comparando el momento de mayor claridad
        (cuando cada quien ya tenia una lectura firme de la situacion) contra ese momento de presion. Si esa
        comparacion es baja, dedica el bloque principal a reconstruir, con frases textuales de al menos dos
        personas, que paso exactamente entre esos dos momentos. Si es alta o media, dilo tambien pero sin
        inventar un quiebre que no ocurrio.
        ...
        en el equipo para que una lectura clara se hiciera pequeña o se suavizara?". Lleva siempre la
        lectura hacia lo sistemico: liderazgo, seguridad para disentir, jerarquia clinica, coordinacion del
        conocimiento distribuido entre especialistas.
        ...
        Un parrafo breve (2 a 4 frases) que generalice el aprendizaje mas alla del ejercicio — la idea de
        que quien coordina no tiene que ser quien mas sabe, sino quien logra que el conocimiento distribuido
        entre varias personas se convierta en una decision conjunta.
    ...
    private const SYSTEM_PROMPT_GLOBAL = <<<'PROMPT'
        ...
        simulacion (un hospital ficticio) en la misma sesion (misma empresa o mismo cliente), cada uno con
        sus propias dimensiones. No recalcules esos numeros — son un hecho dado.
    ...
    private function llamar(string $systemPrompt, string $userPrompt): ?string
    {
        ...
        if ($body === false || $status < 200 || $status >= 300) {
            log_message('error', 'KimiAnalisisCodigoAzul: fallo la llamada (status ' . $status . ')');
            return null;
        }
        ...
        if (!empty($radiografia['dimensiones'])) {
            ...
            if ($persistencia === 'Baja') {
                ...
                $lineas[] = 'La "Persistencia ante presión" salió BAJA: el equipo llegó a su momento de '
                    . 'mayor claridad y luego perdió fuerza cuando el médico tratante, ya con una decisión '
                    . 'inclinada, preguntó "¿tienes algo que cambie esto?" sin aportar ningún dato nuevo. '
                    . 'Ese es el punto de quiebre real de este equipo — reconstrúyelo con las trayectorias '
                    . 'de arriba.';
            }
        }
