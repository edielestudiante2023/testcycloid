...
/**
 * Analisis de patrones de comunicacion para el cierre reflexivo de "Volver
 * a Casa", usando Kimi (Moonshot AI). Mismo patron que KimiAnalisis.php (El
 * Meridian) — es un texto de apoyo para el facilitador, NO un diagnostico
 * psicologico clinico de nadie.
 *
 * La "radiografia" (dimensiones) se calcula matematicamente en
 * VolverACasaAnalisisModel::radiografiaPorEquipo() y se le pasa a Kimi como
 ...
 */
class KimiAnalisisVolverACasa
{
    private const SYSTEM_PROMPT = <<<'PROMPT'
        Eres un facilitador experto en dinamicas de liderazgo y comunicacion organizacional. Analizas los
        datos de un ejercicio de simulacion (un equipo de especialistas de control de mision que debe
        coordinarse tras una emergencia a bordo de una nave ficticia) para preparar al facilitador humano
        antes de su conversacion de cierre real con el equipo.
        ...
        3. Sostener o ceder ante presion: alguien que dio el dato real y completo cuando se le exigio
           rapidez en su lugar, o que cedio dando la version mas rapida o mas comoda.
        ...
        PUNTO DE QUIEBRE: la radiografia te indica si la capacidad del equipo de sostener el dato real bajo
        presion fue alta, media o baja, comparando el momento de mayor claridad del equipo (cuando se
        confirmo la emergencia) contra el momento de mayor presion (cuando el Director de Vuelo exigio una
        cifra ya, sin adivinar). Si esa comparacion es baja, dedica el bloque principal a reconstruir, con
        frases textuales de al menos dos personas, que paso exactamente entre esos dos momentos. Si es alta
        o media, dilo tambien pero sin inventar un quiebre que no ocurrio.
        ...
        en el equipo para que un dato claro se hiciera pequeno o se suavizara?". Lleva siempre la lectura
        hacia lo sistemico: liderazgo, seguridad para disentir, jerarquia, coordinacion del conocimiento
        distribuido entre especialistas.
    ...
    private function llamar(string $systemPrompt, string $userPrompt): ?string
    {
        ...
        if ($body === false || $status < 200 || $status >= 300) {
            log_message('error', 'KimiAnalisisVolverACasa: fallo la llamada (status ' . $status . ')');
            return null;
        }
        ...
        if (!empty($radiografia['dimensiones'])) {
            ...
            if ($persistencia === 'Baja') {
                ...
                $lineas[] = 'La "Persistencia ante presión" salió BAJA: el equipo llegó a su momento de '
                    . 'mayor claridad (cuando se confirmó la emergencia) y luego perdió fuerza (bajo presión '
                    . 'del Director de Vuelo). Ese es el punto de quiebre real de este equipo — '
                    . 'reconstrúyelo con las trayectorias de arriba.';
            }
        }
